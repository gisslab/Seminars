...
\item  Deposits
\begin{itemize}
    \item Deposit quantities: FDIC (branch level, yearly)
    \item Deposit rate: RateWatch (10K Money Market rate)
   \item County-level market
   \end{itemize}
\item  Mortgages
\begin{itemize}
    \item Mortgage quantities: HMDA (lender, loan size, location of property, loan type )
...
\item Property losses from natural disasters from SHELDUS. %(Spatial Hazard Events and Losses Database for the US)
    \end{wideitemize}
\end{frame}
...
$$
 where $\psi_{D, n t}=\log \left(\sum_m \exp \left(\alpha_D R_{D, n m t}+X_{D, n m t} \beta_D+\delta_{D, n m t}\right)\right)$ is the desirability of the composite good, and $\bar{F}_{D, n t}$ is the total supply of funds. 
...
\end{frame}
    ...
    \begin{equation}
        ...
        Analogous expressions for loans, mortgages and reserves. 
